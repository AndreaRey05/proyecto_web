...
function Materias({ rol }) {
    ...
    // Cargar profesores de la materia seleccionada
    useEffect(() => {
        if (seleccionada) {
            fetch(`${API_URL}/api/materias/${seleccionada.id}/profesores`, {
    ...
    const confirmarEliminar = async () => {
        ...
        if (seleccionada?.id === modalEliminar) setSeleccionada(null);
        setModalEliminar(null);
    };

    // Agregar nueva materia
    const handleAgregar = async () => {
        if (!nuevaMateria.nombre.trim() || !nuevaMateria.semestre.trim()) {
        ...
        const data = await res.json();
        if (!res.ok) {
            setErrorAgregar(data.error || 'Error al crear materia');
            return;
        }
                            ...
                            <div className="flex flex-col gap-1 border-b pb-2">
                                ...
                                <div className="flex flex-wrap gap-1">
                                    ...
                                    {profesoresMateria.length === 0 && (
                                        <span className="text-gray-400 text-xs">Sin profesores asignados</span>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            {modalAgregar && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-2xl shadow-xl p-6 w-96 relative flex flex-col gap-4">
                        <button onClick={() => { setModalAgregar(false); setErrorAgregar(''); setNuevaMateria({ nombre: '', semestre: '' }); }}
                            className="absolute top-3 right-4 text-gray-400 hover:text-gray-700 text-xl">✕</button>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                className="border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#5E0006]"
                            />
                        </div>
                        <div className="flex flex-col gap-2">
                        ...
                        <button
                            ...
                            className="bg-[#5E0006] text-white py-2 rounded-lg font-bold text-sm hover:bg-[#7a0008] transition"
                        >
                            Guardar materia
                        </button>
                    </div>
